...
fixed_data = pd.read_csv("result/sim/opt_0.csv")
result_string = ['opt']
p_error_array = np.zeros((len(result_string), len(fixed_data['p_x']), num_realizations))
q_error_array = np.zeros_like(p_error_array)

for l in range(len(result_string)):
    for k in range(num_realizations):
        data = pd.read_csv("result/sim/%s_s_%s.csv" % (result_string[l],k))
        for i in range(len(fixed_data['p_x'])):
            p_error_array[l, i, k] = math.sqrt( (fixed_data['p_x'][i]-data['p_x'][i])**2 + (fixed_data['p_y'][i]-data['p_y'][i])**2
                                                + (fixed_data['p_z'][i]-data['p_z'][i])**2)
            q_error_array[l, i, k] = quat_diff([data['q_w'][i], data['q_x'][i], data['q_y'][i], data['q_z'][i]],
                                               [fixed_data['q_w'][i], fixed_data['q_x'][i], fixed_data['q_y'][i], fixed_data['q_z'][i]])
...
for n in range(len(result_string)):
    ...
    ax1.plot(fixed_data['timestamp'], error_q_m, color = plot_color[result_string[n]], linewidth=line_width, label='error')
    ax1.fill_between(fixed_data['timestamp'], error_q_m - error_q_s/2,
    ...
    ax2.plot(fixed_data['timestamp'], error_p_m, color = plot_color[result_string[n]], linewidth=line_width, label='error')
    ax2.fill_between(fixed_data['timestamp'], error_p_m - error_p_s/2,
