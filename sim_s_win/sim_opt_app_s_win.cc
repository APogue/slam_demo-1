...
public:
    ...
    bool EM_step() {
        ...
        while (!reach_end) {
            ...
            state_para_vec_.at(T)->GetRotationBlock()->setEstimate(state_est_vec_.at(T)->q_);
            state_para_vec_.at(T)->GetVelocityBlock()->setEstimate(state_est_vec_.at(T)->v_);
            state_para_vec_.at(T)->GetPositionBlock()->setEstimate(state_est_vec_.at(T)->p_);
            optimization_problem.AddParameterBlock(state_para_vec_.at(T)->GetRotationBlock()->parameters(), 4,
                                                   quat_parameterization_ptr);
            optimization_problem.AddParameterBlock(state_para_vec_.at(T)->GetVelocityBlock()->parameters(), 3);
            optimization_problem.AddParameterBlock(state_para_vec_.at(T)->GetPositionBlock()->parameters(), 3);
            optimization_problem.SetParameterBlockConstant(state_para_vec_.at(T)->GetRotationBlock()->parameters());
            optimization_problem.SetParameterBlockConstant(state_para_vec_.at(T)->GetVelocityBlock()->parameters());
...
//                state_para_vec_.at(i+1)->GetPositionBlock()->setEstimate(state_est_vec_.at(i+1)->p_);
                ...
                optimization_problem.AddParameterBlock(state_para_vec_.at(i+1)->GetRotationBlock()->parameters(), 4,
                                                       quat_parameterization_ptr);
        ...
        ceres::Solve(optimization_options, &optimization_problem, &optimization_summary);
            ...
            for (size_t i=T; i<T+block_size; ++i) {
                state_est_vec_.at(i+1)->q_ = state_para_vec_.at(i+1)->GetRotationBlock()->estimate();
                state_est_vec_.at(i+1)->v_ = state_para_vec_.at(i+1)->GetVelocityBlock()->estimate();
                state_est_vec_.at(i+1)->p_ = state_para_vec_.at(i+1)->GetPositionBlock()->estimate();
            }
            ...
            T = T + block_size;
            std::cout << "Completed OPT loop "  << T << std::endl;
...
//    Eigen::Rand::Vmt19937_64 urng{(unsigned int) time(0)};
    ...
    for (size_t i = 0; i < num_real; ++i) {
        ...
        slam_problem.OutputResult("result/sim/opt_s_" + std::to_string(i) + ".csv");
